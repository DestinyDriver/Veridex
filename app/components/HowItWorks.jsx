...
const cardStyle = {
  ...
  boxShadow: "0 30px 60px rgba(0,0,0,0.45), inset 0 1px 0 rgba(243,234,216,0.04)",
};

const previewBorderStyle = { border: "1px solid rgba(243,234,216,0.08)" };

function ConnectPreview() {
  const platforms = ["OpenAI", "Anthropic", "Cursor", "GitHub Copilot", "Gemini"];
  return (
    <div className="space-y-2.5">
      {platforms.map((p, i) => (
        <div key={p} className="flex items-center justify-between text-xs font-body">
          <span className="text-[#f3ead8]/85">{p}</span>
          <span className={`text-[10px] uppercase tracking-[0.14em] ${i < 4 ? "text-[#f3ead8]" : "text-[#f3ead8]/40"}`}>
...
function AuditPreview() {
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-[#f3ead8]/70 font-body font-light">12 issues found across 47 seats</p>
    </div>
  );
}
...
const steps = [
  {
    n: "01",
    title: "Connect",
    blurb: "Link the billing accounts your team uses. OpenAI, Anthropic, Cursor, GitHub, and others. Read-only, takes under a minute.",
    Preview: ConnectPreview,
  },
  {
    n: "02",
    title: "Audit",
    blurb: "We map every seat, key, and invoice. Flag the dormant accounts, the wrong tiers, the duplicate tools, the over-list prices.",
    Preview: AuditPreview,
  },
  {
    n: "03",
    title: "Optimize",
    blurb: "Get a prioritized action list, one-click cancellations, and alternative tools with cheaper or volume-priced equivalents.",
...
function MobileCard({ step, delay }) {
  ...
  return (
    <motion.div {...reveal(delay)} className="rounded-[1.5rem] p-7 min-h-[360px] flex flex-col" style={cardStyle}>
      <div className="flex items-baseline justify-between">
        <span className="font-heading italic text-[#f3ead8]/40 text-5xl leading-none">{n}</span>
        <span className="text-[11px] uppercase tracking-[0.18em] text-[#f3ead8]/55 font-body">Step</span>
      </div>
      <h3 className="mt-8 font-heading italic text-[#f3ead8] text-4xl leading-none tracking-[-1px]">{title}</h3>
      <p className="mt-3 text-sm text-[#f3ead8]/70 font-body font-light leading-snug max-w-[34ch]">{blurb}</p>
...
function DesktopCard({ step, cardRef }) {
  ...
  return (
    ...
    >
      <div className="rounded-[1.5rem] p-7 min-h-[420px] flex flex-col" style={cardStyle}>
  ...
  useEffect(() => {
    ...
    Promise.all([import("gsap"), import("gsap/ScrollTrigger")]).then(
      ([{ default: gsap }, { ScrollTrigger }]) => {
        ...
        gsap.set(c1, { xPercent: -50, yPercent: -50, x: -28, rotation: -5, scale: 0.94, zIndex: 1 });
        gsap.set(c2, { xPercent: -50, yPercent: -50, x: 0, rotation: 0, scale: 1.0, zIndex: 3 });
        gsap.set(c3, { xPercent: -50, yPercent: -50, x: 28, rotation: 5, scale: 0.94, zIndex: 2 });
        ...
        tl.to(c1, { x: -360, rotation: -3, scale: 1, zIndex: 2, ease: "power2.inOut" }, 0)
          .to(c3, { x: 360, rotation: 3, scale: 1, zIndex: 2, ease: "power2.inOut" }, 0)
          .to(c2, { scale: 1.02, ease: "power1.out" }, 0)
          .to(contents, { opacity: 1, y: 0, ease: "power2.out", stagger: 0.02 }, 0.25)
          .to(
            [c1, c2, c3],
            {
              boxShadow:
